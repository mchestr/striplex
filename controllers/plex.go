...
// PlexController handles Plex authentication operations.
...
func (p *PlexController) Authenticate(c *gin.Context) {
	...
	params.Add("forwardUrl", fmt.Sprintf("https://%s%s/callback?state=%s",
		config.Config.GetString("server.hostname"), p.basePath, state))
